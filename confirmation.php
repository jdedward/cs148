...
if(isset($_GET["q"])){
    ...
    $sql = "SELECT fldDateJoined, fldUserEmail FROM tblRegisterUser WHERE pkUserID=" . $key2;
    $stmt = $db->prepare($sql);
    $stmt->execute();
    ...
    if($key1==$k1){
        ...
        $sql = "UPDATE tblRegisterUser set fldConfirmed=1 WHERE pkUserID=" . $key2;
        $confirmed = true;
    }
...
if($confirmed){
    ...
    $message =  "<p>I am sorry but this project cannot be confrimed at this time. Please contact us directly.</p>"; 
}
...
$subject="Maxine Davis Glass Art Membership Confirmed";
...
if($confirmed){
    // send an email to the admin, the message of the email is like this message with a link to aprrove
        ...
        //$subject="New Maxine Davis Glass Art Membership: Approve?";
